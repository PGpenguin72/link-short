      ...
      <header className="border-b border-slate-800 px-4 sm:px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {user.is_admin && (
              <RouterLink to="/admin" className="text-xs text-blue-400 hover:text-blue-300">管理後台</RouterLink>
            )}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          ...
          <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-3">
            <input type="text" placeholder="自訂短代碼（選填）" value={newSlug} onChange={(e) => setNewSlug(e.target.value)}
              pattern="[a-zA-Z0-9_\-]*" className="flex-none sm:w-44 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
            ...
            <button type="submit" disabled={creating} className="shrink-0 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 px-5 py-2 rounded-lg text-sm font-medium transition-colors">{creating ? '建立中…' : '建立'}</button>
          </form>
                ...
                editingId === link.id ? (
                  <div key={link.id} className="px-5 py-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleSave(link.id)} disabled={saving} className="text-xs bg-blue-600 hover:bg-blue-500 disabled:opacity-50 px-3 py-1.5 rounded-lg font-medium transition-colors">{saving ? '儲存中…' : '儲存'}</button>
                      <button onClick={() => setEditingId(null)} className="text-xs text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 px-3 py-1.5 rounded-lg transition-colors">取消</button>
                    </div>
                  </div>
                ) : (
                  <div key={link.id} className="px-5 py-4 flex items-center gap-4 group">
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div className="flex items-center gap-2">
                        <a href={shortUrl(link.slug)} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 font-mono text-sm font-medium transition-colors">/{link.slug}</a>
                        {link.active === 0 && (
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => setQrLink(link)} className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-700 rounded-lg transition-colors" title="QR Code / 分享">
                      ...
                      <button onClick={() => startEdit(link)} className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-700 rounded-lg transition-colors" title="編輯">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => handleDelete(link.id, link.slug)} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" title="刪除">
